...
function buildFileOperations(
	...
	if (prevCompactionIndex >= 0) {
		const rawEntry = entries[prevCompactionIndex];
		if (rawEntry?.type === "compaction" && !rawEntry.fromHook) {
			const details = (rawEntry as unknown as { details?: { readFiles?: unknown; modifiedFiles?: unknown } }).details;
