...
export function mergePolicy(base: CompactionPolicy, patch: CompactionPolicyPatch): CompactionPolicy {
	return {
		...
		summary: { ...base.summary, ...(patch.summary ?? {}) },
			...(patch.summaryRetention !== undefined
			? { summaryRetention: patch.summaryRetention }
			: base.summaryRetention !== undefined
			  ? { summaryRetention: base.summaryRetention }
			  : {}),
...
export function applyProfileOverrides(policy: CompactionPolicy, profile: ProfileOverride): CompactionPolicy {
	return {
		...
		...(profile.summaryRetention !== undefined
			? { summaryRetention: profile.summaryRetention }
			: policy.summaryRetention !== undefined
