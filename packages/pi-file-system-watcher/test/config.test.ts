...
describe("loadConfig", () => {
	...
	it("strips defaultDisplayMode when the value is not a supported mode", () => {
...
describe("configFilePath", () => {
	it("resolves under getAgentDir()", () => {
		expect(configFilePath()).toBe("/fake/agent/pi-file-system-watcher.json");
	});
});
...
describe("saveConfig", () => {
	...
	it("returns true on a clean write", () => {
		...
		expect(path).toBe("/fake/agent/pi-file-system-watcher.json");
		expect(JSON.parse(body as string)).toEqual({ defaultDisplayMode: "statusline" });
		expect((body as string).endsWith("\n")).toBe(true);
	});

	it("merges over existing file and preserves unknown keys", () => {
		vi.mocked(readFileSync).mockReturnValue(
