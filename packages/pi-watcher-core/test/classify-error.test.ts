...
// Plain error helpers — no typed classes needed since predicates match by .name
const authErr      = (msg: string) => Object.assign(new Error(msg), { name: "TestAuthError" });
const throttleErr  = (msg: string) => Object.assign(new Error(msg), { name: "TestThrottleError" });
const notFoundErr  = (msg: string) => Object.assign(new Error(msg), { name: "TestNotFoundError" });
const genericErr   = (msg: string) => Object.assign(new Error(msg), { name: "TestWatcherError" });

const opts = {
	authPredicate:     (e: unknown) => (e as Error).name === "TestAuthError",
	throttlePredicate: (e: unknown) => (e as Error).name === "TestThrottleError",
	notFoundPredicate: (e: unknown) => (e as Error).name === "TestNotFoundError",
};
...
describe("classifyWatcherError", () => {
	it.each([
		{
			name: "auth_error",
			err: authErr("session expired svc.example.internal#X"),
		...
		{
			name: "throttle_error",
			err: throttleErr("slow down svc.example.internal#Throttled"),
		...
		},
		{
			...
			err: notFoundErr("missing svc.example.internal#X"),
		...
		},
		{
			...
			err: genericErr('Unexpected response: {"__type":"svc.example.internal#Oops"} <html><body>sensitive</body></html>'),
	...
	it("user_message_is_literal_default_not_derived_from_err_message", () => {
		const err = authErr("Received HTML instead of JSON — session expired. <!DOCTYPE html>");
		const result = classifyWatcherError(err, opts);
	...
	it("auth_check_takes_priority_over_generic_error", () => {
		// auth predicate fires before generic fallthrough.
		const err = authErr("y");
		const result = classifyWatcherError(err, opts);
...
describe("classifyWatcherError — predicate support", () => {
	it("authPredicate matches when error is not an AuthError instance", () => {
		...
		const result = classifyWatcherError(err, {
			...
			throttlePredicate: (e) => (e as Error).name === "TestThrottleError",
		});
		expect(result.kind).toBe("auth");
	...
	it("throttlePredicate matches when error is not a ThrottleError instance", () => {
		...
		const result = classifyWatcherError(err, {
			authPredicate:     (e) => (e as Error).name === "TestAuthError",
