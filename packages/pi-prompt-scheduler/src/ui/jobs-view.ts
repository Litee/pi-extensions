...
/**
 * JobsView — single TUI overlay that consolidates browse + toggle + remove
 * + cleanup for scheduled prompts. Jobs are added exclusively via the
 * `schedule_prompt` tool (LLM-driven); there is no manual add flow here.
 * Foreign-session jobs render read-only in a separate group; actions (t/x/c)
 * ignore them.
 */
...
export class JobsView implements Component {
  ...
  private handleNormalInput(data: string): void {
    ...
    if (matchesKey(data, "s")) {
      ...
      if (this.isSelectionForeign()) return;
      // Toggle session binding: bound → unbound and vice versa.
      // Build the updated job carefully to satisfy exactOptionalPropertyTypes.
      if (sel.session) {
        ...
        this.storage.updateJob(sel.id, { ...(session !== undefined ? { session } : {}) });
        const updated: CronJob = { ...sel, ...(session !== undefined ? { session } : {}) };
        this.scheduler.updateJob(sel.id, updated);
      }
      this.refresh();
      return;
    }
  ...
  render(width: number): string[] {
    ...
    if (this.confirm?.kind === "remove") {
      ...
      lines.push(` ${this.theme.fg("accent", this.theme.bold("Jobs"))}  —  q quit`);
    } else {
      lines.push(
        ` ${this.theme.fg("accent", this.theme.bold("Jobs"))}  —  ↑↓ select   t toggle   s scope   x remove   c cleanup   q quit`,
      );
    }
    lines.push(rule);
    ...
    if (total === 0) {
      lines.push("");
      lines.push("  No scheduled prompts. Ask the LLM to schedule one via the `schedule_prompt` tool, or press `q` to quit.");
      lines.push("");
      lines.push(rule);
      return lines;
    }
