...
export class CronScheduler {
  ...
  private executeJobInSubagent(job: CronJob): void {
    ...
    // empty content — if marker lands right after assistant message,
    // conversation ends on assistant. Anthropic 400s.
    this.pi.sendMessage({
      customType: "scheduled_prompt",
      ...
      try {
        ...
        if (result.ok) {
          ...
          try {
            ...
            // notify=false: content still non-empty (guarded by || fallback)
            // to prevent session poisoning — pi-ai filters empty user messages,
            // leaving conversation ending on assistant which Anthropic 400s.
            // No delivery options means marker is silent (parent not woken).
            this.pi.sendMessage(
              {
                customType: "scheduled_prompt",
          ...
          try {
            // Same notify-gated wake-up done marker — see comment above.
            this.pi.sendMessage(
              {
                customType: "scheduled_prompt",
