...
const _hasExplicitBgConfig = false;
...
export default async function diffRendererExtension(pi: ExtensionAPI): Promise<void> {
	...
	// getMarkdownTheme / MarkdownComponent from the SDK are intentionally not
	// destructured here: they are unused by pi-diff (dropped for the monorepo's
	// noUnusedLocals). TextComponent is the only TUI component we render with.
			...
			: typeof metaOrHeader === "function"
				...
				: (width: number) =>
						formatToolFrameHeader({
							...
							...(frame?.headerLeftPad !== undefined ? { headerLeftPad: frame.headerLeftPad } : {}),
						});
	...
	function getEditOperations(input: any): Array<{ oldText: string; newText: string }> {
		if (Array.isArray(input?.edits)) {
			return input.edits
				...
				.filter((edit: { oldText: string; newText: string }) => edit.oldText && edit.oldText !== edit.newText) as Array<{ oldText: string; newText: string }>;
		}
			...
			if (fp && operations.length > 0 && existsSync(fp)) {
				try {
					...
					if (replaceOk) {
						...
						const merged: NonNullable<(typeof diffs)[0]> = {
	...
	registerToolIfEnabled("apply_patch", {
		...
		renderResult(result: any, _opt: any, theme: any, ctx: any) {
			const text = getWidthAwareText(ctx.lastComponent);
			if (ctx.isError) {
				const out = (result.content ?? []).map((c: { type: string; text?: string }) => c.text ?? "").join("\n") || "Error";
				delete text.__piDiffTask;
				setToolErrorBg(text, theme);
