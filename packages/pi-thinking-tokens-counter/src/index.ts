...
// Local type for the patched ctx.ui method — not in published types
interface ThinkingTokensUI {
    setThinkingTokenCount(count: number, options?: { messageId?: number; rate?: number }): void;
}
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("message_update", (event, ctx) => {
    ...
    if (chars > thinkingCharCount) {
      ...
      (ctx.ui as ThinkingTokensUI).setThinkingTokenCount(estimatedTokens, { messageId: currentMessageId, rate });
    }
  });
}
