...
export type FleetUICtx = {
  setWidget(
    key: string,
    content: undefined | ((tui: TUI, theme: Theme) => { render(width: number): string[]; invalidate(): void; dispose?(): void }),
  ...
  custom<T>(
    factory: (tui: TUI, theme: Theme, keybindings: ViewerKeybindings | undefined, done: (result: T) => void) => { render(width: number): string[]; invalidate(): void; dispose?(): void },
