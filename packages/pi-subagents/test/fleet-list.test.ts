...
import { getDisplayName, type Theme } from "../src/ui/agent-widget.js";
...
function makeRecord(over: Partial<AgentRecord> = {}): AgentRecord {
  return {
    ...
    toolUses: 0,
    startedAt: Date.now(),
    session: FAKE_SESSION as unknown as AgentSession,
...
function harness(agents: AgentRecord[]): Harness {
  ...
  let widgetFactory: ((tui: TUI, theme: Theme) => { render(w: number): string[]; invalidate(): void; dispose?(): void }) | undefined;
  let editorText = "";
  let opened = false;
  let closed = false;
  ...
  const ui: FleetUICtx = {
    ...
    notify: () => {},
    custom: ((factory: (tui: TUI, theme: Theme, keybindings: ViewerKeybindings | undefined, done: (result: undefined) => void) => { render(width: number): string[]; invalidate(): void; dispose?(): void }) => {
      opened = true;
      return new Promise<undefined>((resolve) => {
        ...
        factory(fakeTui as unknown as TUI, theme, undefined, done);
      });
  ...
  return {
    ...
    render: (width = 120) => (widgetFactory ? widgetFactory(fakeTui as unknown as TUI, theme).render(width) : []),
...
describe("FleetList overlay lifecycle", () => {
  ...
  it("keeps the cursor on the viewed agent after closing, even if the list reordered", async () => {
    ...
    const agents = [
      makeRecord({ id: "a1", description: "one", session: fakeSession as unknown as AgentSession }),
      makeRecord({ id: "a2", description: "two", session: fakeSession as unknown as AgentSession }),
      makeRecord({ id: "a3", description: "three", session: fakeSession as unknown as AgentSession }),
    ];
    const h = harness(agents);
