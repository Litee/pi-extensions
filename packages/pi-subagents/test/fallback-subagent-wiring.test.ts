...
function makePi() {
  ...
  const pi = {
    ...
    registerTool: vi.fn((tool: { execute: (...args: unknown[]) => Promise<WireToolResult>; name: string }) => tools.set(tool.name, tool)),
...
describe("fallbackSubagent gates dispatch through the real Agent tool", () => {
  ...
  afterEach(() => {
    ...
    delete (globalThis as unknown as Record<symbol, unknown>)[Symbol.for("pi-subagents:manager")];
    vi.restoreAllMocks();
  ...
  it("still falls back — and says so — when the setting is unset", async () => {
    ...
    vi.mocked(runAgent).mockResolvedValue({
      responseText: "done", session: { dispose: vi.fn() } as unknown as import("@earendil-works/pi-coding-agent").AgentSession, aborted: false, steered: false,
    });
  ...
  it("carries the fallback note on the background branch too", async () => {
    ...
    const { tools } = boot();
    vi.mocked(runAgent).mockReturnValue(new Promise<never>(() => {}));
  ...
  it("never persists a blank type into a scheduled job", async () => {
    ...
    const jobs = readdirSync(storeDir).flatMap((f) =>
      (JSON.parse(readFileSync(join(storeDir, f), "utf-8")) as { jobs?: Array<{ subagent_type?: string }> }).jobs ?? [],
    );
  ...
  it("never blocks resume, which ignores subagent_type entirely", async () => {
    ...
    const { tools } = boot();
    vi.mocked(runAgent).mockResolvedValue({
      responseText: "first", session: { dispose: vi.fn() } as unknown as import("@earendil-works/pi-coding-agent").AgentSession, aborted: false, steered: false,
    });
  ...
  it("applies the same contract to cross-extension spawns", () => {
    ...
    boot();
    setFallbackSubagent(NO_FALLBACK);
    const registry = (globalThis as unknown as Record<symbol, { spawn: (...args: unknown[]) => unknown }>)[Symbol.for("pi-subagents:manager")]!;
