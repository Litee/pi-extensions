...
type TestRecord = { id: string; type?: string; status: string; parentAgentId?: string | undefined; result?: string | undefined; outputFile?: string; pendingSteers?: string[]; promise?: Promise<unknown> | undefined; session?: { steer: unknown }; rootSessionId?: string };
...
type NestedToolResult = { content: Array<{ text?: string }>; isError?: boolean; details?: Record<string, unknown> };
function execute(tool: ToolDefinition, params: Record<string, unknown>, executionCwd = cwd): Promise<NestedToolResult> {
  return tool.execute("call-1", params, undefined, undefined, ctx(executionCwd)) as unknown as Promise<NestedToolResult>;
}
...
beforeEach(() => {
  ...
  records = new Map();
  spawn = vi.fn((_pi: unknown, _ctx: unknown, type: string, _prompt: string, options: { parentAgentId?: string }) => {
    const id = `child-${records.size + 1}`;
    records.set(id, { id, type, status: "running", parentAgentId: options.parentAgentId ?? undefined });
    return id;
  });
  spawnAndWait = vi.fn((_pi: unknown, _ctx: unknown, type: string, _prompt: string, options: { parentAgentId?: string }) => {
    const id = `child-${records.size + 1}`;
    const record: TestRecord = { id, type, status: "completed", result: "done", parentAgentId: options.parentAgentId ?? undefined };
...
describe("child-safe nested Agent tools", () => {
  ...
  it("supports background launches and ownership-scopes result, resume, and steer", async () => {
    ...
    expect((manager as unknown as { resume: () => unknown }).resume).not.toHaveBeenCalled();
  });
  ...
  it("flags a truncated child run instead of passing partial output off as complete", async () => {
    spawnAndWait.mockImplementation(() => ({
      id: "child-1",
  ...
  it("attributes a nested child's token spend to the owning parent", async () => {
    ...
    spawn.mockImplementation((_pi: unknown, _ctx: unknown, _type: string, _prompt: string, options: { onAssistantUsage?: (usage: { input: number; output: number; cacheWrite: number }) => void }) => {
  ...
  it("files a nested transcript under the root session, honoring output_transcript", async () => {
    ...
    spawnAndWait.mockImplementation((_pi: unknown, _ctx: unknown, type: string, _prompt: string, options: { parentAgentId?: string }, onSpawned?: (id: string) => void) => {
      const record: TestRecord = { id: "child-1", type, status: "completed", result: "done", parentAgentId: options.parentAgentId };
      records.set("child-1", record);
      onSpawned?.("child-1");
