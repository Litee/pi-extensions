...
describe("AgentManager — spawnAndWait onSpawned + foreground output file wiring (#105)", () => {
  ...
  it("fields set on the record in onSpawned are visible when onSessionCreated fires", async () => {
    ...
    vi.mocked(runAgent).mockImplementation(async (_ctx: unknown, _type: string, _prompt: string, opts: RunOptions) => {
  ...
  it("restores the shared onSpawned callback before awaiting the foreground run", async () => {
    ...
    vi.mocked(runAgent)
      .mockImplementationOnce(() => new Promise(resolve => { finishFirst = resolve as (value: unknown) => void; }))
...
describe("AgentManager — nested runtime propagation", () => {
  ...
  it("starts a nested background child even when the concurrency pool is full", () => {
  ...
  it("aborts owned children when the parent settles", async () => {
    ...
    const abortable = (_ctx: unknown, _type: string, _prompt: string, opts: RunOptions) =>
    ...
    vi.mocked(runAgent)
      .mockImplementationOnce(() => new Promise(resolve => { finishParent = resolve as (value: unknown) => void; }))
      .mockImplementation(abortable);
    manager = new AgentManager();
  ...
  it("aborts children spawned during a resumed turn", async () => {
    ...
    vi.mocked(resumeAgent).mockImplementation(() => {
      ...
      return Promise.resolve({ text: "resumed" });
    });
...
describe("AgentManager — lifetime usage + compaction count are eagerly initialized", () => {
  ...
  it("onAssistantUsage from runAgent accumulates into record.lifetimeUsage", async () => {
    ...
    vi.mocked(runAgent).mockImplementation((_ctx: unknown, _type: string, _prompt: string, opts: RunOptions) => {
  ...
  it("resume() also accumulates usage and increments compactions on the same record", async () => {
    ...
    vi.mocked(resumeMock).mockImplementation((_session: unknown, _prompt: string, opts: Parameters<typeof resumeMock>[2]) => {
...
describe("AgentManager — steer()", () => {
  ...
  it("delivers to a live session via session.steer()", () => {
    ...
    let captured: ((s: import("@earendil-works/pi-coding-agent").AgentSession) => void) | undefined;
    vi.mocked(runAgent).mockImplementation((_ctx: unknown, _type: string, _prompt: string, opts: RunOptions) => {
    ...
    captured?.({ steer, dispose: vi.fn() } as unknown as import("@earendil-works/pi-coding-agent").AgentSession);
