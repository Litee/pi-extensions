...
interface MockTool { name: string; execute(...args: unknown[]): Promise<unknown> }
type MockHandler = (...args: unknown[]) => unknown;
type MockResult = { content: { text: string }[] };

function makePi() {
  const tools = new Map<string, MockTool>();
  const lifecycle = new Map<string, MockHandler>(); // pi.on(...) — session_start, session_before_switch, session_shutdown
  const events = new Map<string, MockHandler>(); // pi.events.on(...) — subagents:rpc:*, etc.
  const pi = {
    registerMessageRenderer: vi.fn(),
    registerTool: vi.fn((t: MockTool) => tools.set(t.name, t)),
    registerCommand: vi.fn(),
    on: vi.fn((event: string, handler: MockHandler) => lifecycle.set(event, handler)),
    events: {
      emit: vi.fn(),
...
const textOf = (r: unknown): string => (r as MockResult).content[0]!.text;
...
async function spawnCompletedBackgroundAgent(tools: Map<string, MockTool>): Promise<string> {
...
describe("issue #108: unread completed background agents survive session events", () => {
  ...
  beforeEach(() => {
    ...
    // Spy on process.cwd() so loadSettings() picks up the temp dir without process.chdir()
