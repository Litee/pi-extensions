...
const {
  ...
  const loaderState: LoaderState = { extensions: [], errors: [], runtime: {} };
  return {
...
vi.mock("@earendil-works/pi-coding-agent", () => ({
  ...
  DefaultResourceLoader: class {
    ...
    reload() {
      ...
      const override = this.opts["extensionsOverride"] as ((result: unknown) => unknown) | undefined;
      if (override) {
        loaderExtensionsRef.current = override(loaderExtensionsRef.current) as LoaderState;
      }
    }
...
vi.mock("../src/env.js", () => ({
  detectEnv: vi.fn(() => ({ isGitRepo: false, branch: "", platform: "linux" })),
}));
...
import { getMemoryToolNames, getReadOnlyMemoryToolNames } from "../src/agent-types.js";
...
type SessionOpts = { tools?: string[]; excludeTools?: string[]; customTools?: Array<{ name: string }> };
...
function createSession(finalText: string) {
  ...
  const session = {
    ...
    getAllTools: vi.fn(() => {
      const opts = createAgentSession.mock.calls[0]?.[0] as Record<string, unknown> | undefined;
    ...
    agent: { beforeToolCall: undefined } as unknown as {
      beforeToolCall?: (context: { toolCall: { name: string } }, signal?: AbortSignal) => Promise<unknown>;
    },
...
describe("agent-runner final output capture", () => {
  ...
  it("binds extensions before prompting", async () => {
    ...
    expect(session.bindExtensions).toHaveBeenCalledWith(
      expect.objectContaining<{ onError: unknown }>({ onError: expect.any(Function) }),
    );
  ...
  it("passes the parent model runtime while retaining the legacy model registry", async () => {
    ...
    await runAgent(context as unknown as ExtensionContext, "Explore", "Say AUTHENTICATED", { pi });
  ...
  it("suppresses AGENTS.md/CLAUDE.md/APPEND_SYSTEM.md for subagents", async () => {
    ...
    expect(defaultResourceLoaderCtor).toHaveBeenCalledWith(
      expect.objectContaining<{ noContextFiles: boolean; appendSystemPromptOverride: unknown }>({
        noContextFiles: true,
        appendSystemPromptOverride: expect.any(Function),
      }),
    );
    // The override returns an empty list so any loaded sources are discarded.
    const ctorArgs = (defaultResourceLoaderCtor.mock.calls as unknown[][])[0]![0] as { appendSystemPromptOverride: (sources: string[]) => string[] };
...
describe("agent-runner failed-final-turn detection (#144)", () => {
  ...
  function sessionEnding(...messages: unknown[]) {
  ...
  it("resume whose new turn fails empty does NOT return the previous turn's answer (#144)", async () => {
    ...
    const result = await resumeAgent(session as unknown as AgentSession, "follow-up");
...
describe("agent-runner usage callback wiring", () => {
  function emitMessageEnd(listeners: Array<(e: unknown) => void>, usage: Record<string, unknown> | undefined) {
 ...
 */
function mockRegistry(opts: Record<string, unknown>): string[] {
  const excluded = new Set<string>((opts["excludeTools"] as string[] | undefined) ?? []);
  // pi registers customTools into the same registry, subject to the same gate.
  const customNames: string[] = ((opts["customTools"] as { name: string }[] | undefined) ?? []).map((t) => t.name);
  const tools = opts["tools"] as string[] | undefined;
  const all: string[] = tools
    ? [...tools, ...customNames]
    : [
        ...BUILTINS_7,
...
function lastToolsPassed(): string[] {
  const opts = createAgentSession.mock.calls[0]![0] as SessionOpts;
  if (opts["tools"]) return opts["tools"];
  return lastSession?.getActiveToolNames() ?? [];
}

function lastLoaderOpts(): Record<string, unknown> {
  return (defaultResourceLoaderCtor.mock.calls as unknown[][])[0]![0] as Record<string, unknown>;
}
...
describe("agent-runner master tool allowlist", () => {
  ...
  it("keeps nested tools unavailable without explicit opt-in", async () => {
    ...
    await runAgent(ctx, "Explore", "go", {
      pi,
      nestedRuntime: { manager: {} as unknown as NestedAgentManager, parentAgentId: "parent", depth: 1 },
    });

    expect(createNestedSubagentTools).not.toHaveBeenCalled();
    expect(lastToolsPassed()).not.toContain("Agent");
    expect((createAgentSession.mock.calls[0]![0] as SessionOpts).customTools).toEqual([]);
  });
  ...
  it("injects scoped nested tools for an opted-in non-isolated agent", async () => {
    ...
    const { session } = createSession("OK");
    createAgentSession.mockResolvedValue({ session });
    const manager = {} as unknown as NestedAgentManager;
    ...
    expect((createAgentSession.mock.calls[0]![0] as SessionOpts).customTools).toHaveLength(3);
  });
  ...
  it("passes the inherited depth cap through to the nested tools", async () => {
    ...
    await runAgent(ctx, "Explore", "go", {
      pi,
      nestedRuntime: { manager: {} as unknown as NestedAgentManager, parentAgentId: "parent", depth: 1, maxSubagentDepth: 3 },
    });
  ...
  it("injects no nested tools once the effective cap is reached", async () => {
    ...
    await runAgent(ctx, "Explore", "go", {
      pi,
      nestedRuntime: { manager: {} as unknown as NestedAgentManager, parentAgentId: "parent", depth: 1, maxSubagentDepth: 1 },
    });
...
describe("agent-runner async extension tool registration", () => {
  ...
  it("beforeToolCall preserves a hook pi installed before us", async () => {
    ...
    const { session } = createSession("OK");
    const prior = vi.fn<() => Promise<void>>(() => Promise.resolve());
  ...
  it("scope outlives runAgent so resumed turns stay narrowed", async () => {
    ...
    await runAgent(ctx, "Explore", "go", { pi });
    await resumeAgent(session as unknown as AgentSession, "keep going");
  ...
  it("isolated keeps the static allowlist — no live scoping installed", async () => {
    ...
    expect((createAgentSession.mock.calls[0]![0] as SessionOpts).tools).toEqual(["read"]);
...
describe("agent-runner extension allowlist", () => {
  ...
  it("warns but proceeds when a bare name matches no loaded extension", async () => {
    ...
    const { session } = createSession("OK");
    createAgentSession.mockResolvedValue({ session });
    const onToolActivity = vi.fn<(activity: { toolName?: string }) => void>();
...
describe("agent-runner exclude_extensions", () => {
  ...
  function extensionErrors(onToolActivity: ReturnType<typeof vi.fn<(activity: { toolName?: string }) => void>>): string[] {
...
describe("agent-runner — extensions as array", () => {
  it("filters active tools to only those matching the extension prefix list", async () => {
    ...
    // Tools are scoped live (#157): the allowlist is unset, the active set is
    // repaired post-bind. The mock loader returns no extensions, so the active
    // set is the default built-ins plus nothing ext.
    const tools = lastToolsPassed();
    expect(tools).toContain("read");
    expect(tools).not.toContain("Agent");
    expect(tools).not.toContain("other-ext:tool");
  });
});
...
describe("agent-runner — resumeAgent with no callbacks", () => {
  ...
  it("works when called with abort signal", async () => {
    ...
    expect(result.text).toBe("SIG-RESUME");
  });
});
...
describe("agent-runner — extensions=true (not array, not false)", () => {
  it("does not filter tools to an allowlist when extensions=true", async () => {
    ...
    // With extensions=true, tools are scoped live (#157): builtins pass through,
    // EXCLUDED_TOOL_NAMES stay out, and the mock loader adds no extension tools.
    const tools = lastToolsPassed();
    expect(tools).toContain("read");
    expect(tools).not.toContain("Agent");
    expect(tools).not.toContain("other-ext:tool");
...
describe("agent-runner — resumeAgent with no callbacks (else branch)", () => {
  it("works when called with empty options — the : () => {} branch fires", async () => {
    ...
    const result = await resumeAgent(session as unknown as AgentSession, "go", {});
    expect(result.text).toBe("MIN-RESUME");
  });
  ...
  it("works with only signal callback (no onToolActivity/onAssistantUsage/onCompaction)", async () => {
    ...
    expect(result.text).toBe("SIGNAL-ONLY");
  });
});
