...
function deferredRuns() {
  ...
  vi.mocked(runAgent).mockImplementation(
    () =>
      ...
      }) as unknown as ReturnType<typeof runAgent>,
  );
  return resolvers;
}

async function spawnBackground(tools: Map<string, WireTool>): Promise<{ id: string; queued: boolean }> {
...
describe("get_subagent_result wait:true on a queued agent", () => {
  it("waits through queue start and returns the result (no 'still running')", async () => {
    ...
    expect(JSON.stringify((pi as unknown as { sendMessage: { mock: { calls: unknown[] } } }).sendMessage.mock.calls)).not.toContain(queuedId);
  ...
  it("aborts a running result wait without aborting or consuming the child", async () => {
    ...
    expect((pi as unknown as { sendMessage: () => unknown }).sendMessage).toHaveBeenCalledTimes(1);
  ...
  it("aborts a queued result wait before the agent starts", async () => {
    ...
    let completedResult: WireToolResult | undefined;
