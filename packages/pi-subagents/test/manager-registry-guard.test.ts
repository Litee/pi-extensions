...
type WireToolResult = { content: Array<{ text?: string }>; isError?: boolean; details?: Record<string, unknown> };
type WireTool = { execute: (...args: unknown[]) => Promise<WireToolResult>; name: string };
...
function registrySlot(): ManagerRegistry | undefined {
  return (globalThis as unknown as Record<symbol, ManagerRegistry | undefined>)[MANAGER_KEY];
}

function setRegistrySlot(v: ManagerRegistry | undefined): void {
  const slot = globalThis as unknown as Record<symbol, ManagerRegistry | undefined>;
  if (v === undefined) delete slot[MANAGER_KEY];
  else slot[MANAGER_KEY] = v;
}
...
function makePi() {
  ...
  const pi = {
    ...
    registerTool: vi.fn((tool: WireTool) => tools.set(tool.name, tool)),
...
async function spawnBackground(tools: Map<string, WireTool>): Promise<string> {
...
const priorGlobal = registrySlot();
afterEach(() => {
  if (priorGlobal === undefined) setRegistrySlot(undefined);
  else setRegistrySlot(priorGlobal);
...
describe("Symbol.for manager registry across activations", () => {
  it("child activation does not overwrite the root entry; child shutdown does not delete it", async () => {
    ...
    expect(registrySlot()).toBe(rootEntry);
    expect(registrySlot()!.getRecord(id)).toBeDefined();
    ...
    expect(registrySlot()).toBeUndefined();
  });
});
