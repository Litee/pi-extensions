...
function makePi() {
  ...
  const lifecycle = new Map<string, (...args: unknown[]) => unknown>(); // pi.on(...) — session_start, session_shutdown, …
  const busHandlers = new Map<string, (raw: unknown) => unknown>(); // pi.events.on(...) — rpc channels
  const pi = {
    registerMessageRenderer: vi.fn(),
    registerTool: vi.fn((tool: { execute: (...args: unknown[]) => unknown; name: string }) => tools.set(tool.name, tool)),
...
type MockPi = ReturnType<typeof makePi>["pi"];
function emitCalls(pi: MockPi): Array<Array<unknown>> {
  return (pi as unknown as { events: { emit: { mock: { calls: Array<Array<unknown>> } } } }).events.emit.mock.calls;
}
function onCalls(pi: MockPi): Array<Array<unknown>> {
  return (pi as unknown as { events: { on: { mock: { calls: Array<Array<unknown>> } } } }).events.on.mock.calls;
}
const readyEmits = (pi: MockPi): unknown[] => emitCalls(pi).filter((c) => c[0] === "subagents:ready");
const onCallsFor = (pi: MockPi, channel: string): unknown[] => onCalls(pi).filter((c) => c[0] === channel);
...
describe("issue #142: RPC handlers + subagents:ready are gated on session_start", () => {
  ...
  it("advertises and registers RPC on session_start, and spawn works once bound", async () => {
    ...
    vi.mocked(runAgent).mockImplementation(() => new Promise<never>(() => {})); // never resolves
    ...
    const reply = emitCalls(pi).find(
      (c) => c[0] === `subagents:rpc:spawn:reply:${requestId}`,
    );
    expect(reply, "spawn emitted a reply").toBeTruthy();
    const replyPayload = reply![1] as { success?: boolean; data?: { id?: string } };
    expect(replyPayload.success, `spawn succeeded, got: ${JSON.stringify(reply![1])}`).toBe(true);
    expect(replyPayload.data?.id).toBeTruthy();
  });
