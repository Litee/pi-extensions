...
describe("loadConfig", () => {
	...
	it("returns { defaultDisplayMode: 'widget' } when valid", () => {
	...
	it("returns { defaultDisplayMode: 'statusline' } when valid", () => {
	...
	it("strips defaultDisplayMode when value is unsupported", () => {
		vi.mocked(readFileSync).mockReturnValue(JSON.stringify({ defaultDisplayMode: "inline" }));
		expect(loadConfig()).toEqual({});
	});

	it("strips defaultDisplayMode when value is not a string", () => {
...
describe("configFilePath", () => {
	it("resolves under getAgentDir()", () => {
		expect(configFilePath()).toBe("/fake/agent/pi-aws-ec2-watcher.json");
	});
});
...
describe("saveConfig", () => {
	...
	it("returns true on a clean write and persists merged JSON to the agent dir", () => {
		...
		expect(path).toBe("/fake/agent/pi-aws-ec2-watcher.json");
