/**
 * Ec2Widget — TUI panel rendered below the chat editor.
 *
...
export class Ec2Widget {
	...
	constructor(
		...
		this.unsubscribe = this.pi.events.on("ec2:change", () => this.refresh());
	}
	...
	private renderWidget(width: number, theme: unknown): string[] {
		...
		container.addChild(
			new Text(
				t.fg("accent", t.bold("AWS EC2 Instance Watcher")) +
