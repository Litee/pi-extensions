...
is no repeating stream. On timeout, one chat notification is injected and the watch is marked terminal.
All watches have a maximum duration of 72 h — `timeoutSeconds` defaults to
72 h if omitted and is silently capped at 72 h if higher.
