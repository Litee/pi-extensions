...
function setup(args: {
	...
	const pi = {
		on: vi.fn((eventName: string, cb: (event: unknown, ctx: unknown) => void) => {
	...
	const runtime = {
		...
		ensureConfig: vi.fn(),
		resolveModel: vi.fn((): Promise<ResolveResult> => Promise.resolve({ ok: true, model: { reasoning: true }, apiKey: "key", headers: { h: "v" } })),
		launchConsolidationTask: vi.fn((_ctx: unknown, work: (() => Promise<void>) | undefined) => {
			runtime.consolidationInFlight = true;
			launchedWork = work;
			return Promise.resolve();
		}),
		recordConsolidationStageError: vi.fn((ctx: { ui?: { notify: (message: string, level?: string) => void } }, phase: "observer" | "reflector" | "dropper", error: unknown) => {
	...
	registerConsolidationTrigger(pi as unknown as ExtensionAPI, runtime as unknown as Runtime);
