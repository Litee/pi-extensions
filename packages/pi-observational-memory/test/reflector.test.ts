...
function fakeAgentLoop(handler: (prompts: AgentMessage[], context: AgentContext, config: AgentLoopConfig) => Promise<void> | void): AgentLoopFn {
...
describe("V3 reflector agent", () => {
	...
	it("renders coverage tiers in every active observation line for the reflector", async () => {
		...
		const loop = fakeAgentLoop((prompts) => {
			userText = (prompts[0]! as { content: Array<{ text: string }> }).content[0]!.text;
		});
	...
	it("records one-line V3 reflections with code-computed ids and token counts", async () => {
		...
		const loop = fakeAgentLoop(async (_prompts, context) => {
			await (context.tools![0]!.execute as (id: string, params: unknown) => Promise<unknown>)("tool-1", {
