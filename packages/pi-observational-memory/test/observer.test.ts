...
import type { agentLoop, AgentContext, AgentLoopConfig, AgentMessage, ShouldStopAfterTurnContext } from "@earendil-works/pi-agent-core";
...
function fakeAgentLoop(
	handler: (prompts: AgentMessage[], context: AgentContext, config: AgentLoopConfig) => Promise<void> | void,
): typeof agentLoop {
	return ((prompts: AgentMessage[], context: AgentContext, config: AgentLoopConfig) => ({
	...
	})) as unknown as typeof agentLoop;
}
...
describe("runObserver", () => {
	...
	it("uses maxTurns as an observer turn cap", async () => {
		let shouldStopAfterTurn: ((ctx: ShouldStopAfterTurnContext) => boolean | Promise<boolean>) | undefined;
		...
		expect(shouldStopAfterTurn!({} as ShouldStopAfterTurnContext)).toBe(false);
		expect(shouldStopAfterTurn!({} as ShouldStopAfterTurnContext)).toBe(true);
	});
	...
	it("uses configured observer thinking level for reasoning models", async () => {
		...
		await runObserver({ ...baseArgs, model: { reasoning: true } as unknown as Model<"openai-completions">, agentLoop: loop, thinkingLevel: "minimal" });
	...
	it("omits observer reasoning when thinkingLevel is off", async () => {
		...
		await runObserver({ ...baseArgs, model: { reasoning: true } as unknown as Model<"openai-completions">, agentLoop: loop, thinkingLevel: "off" });
