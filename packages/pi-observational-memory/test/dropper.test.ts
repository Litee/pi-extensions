...
import type { agentLoop, AgentContext, AgentLoopConfig, AgentMessage } from "@earendil-works/pi-agent-core";
...
describe("V3 dropper agent", () => {
	...
	it("passes target-return max drops as a hard upper bound", async () => {
		...
		const loop = fakeAgentLoop((prompts) => {
			userText = (prompts[0] as unknown as { content: Array<{ text: string }> }).content[0]!.text;
		});
