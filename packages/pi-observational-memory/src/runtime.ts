...
export class Runtime {
	...
	async resolveModel(ctx: ResolveCtx): Promise<ResolveResult> {
		...
		const auth = (await ctx.modelRegistry.getApiKeyAndHeaders(typedModel)) as Auth;
		if (!auth.ok || !auth.apiKey) {
			const provider = typedModel.provider;
			return { ok: false, reason: `no API key for provider "${provider}"` };
		}
		return { ok: true, model, apiKey: auth.apiKey, ...(auth.headers ? { headers: auth.headers } : {}) };
	}
