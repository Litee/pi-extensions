...
export async function runObserver(args: RunObserverArgs): Promise<Observation[] | undefined> {
	...
	const recordObservations: AgentTool<typeof RecordObservationsSchema> = {
		...
		execute: (_id: string, params: RecordObservationsArgs) => {
			let added = 0;
			let duplicates = 0;
			let rejected = 0;
			...
			return Promise.resolve({ content: [{ type: "text", text: ack }], details: { added, duplicates, rejected, total: accumulated.size } });
		},
	};
...
${conversation}`;
	...
	const context: AgentContext = {
		...
		messages: [],
		tools: [recordObservations as AgentTool<TSchema>],
	};
