...
export async function runDropper(args: RunDropperArgs): Promise<string[] | undefined> {
	...
	const dropObservations: AgentTool<typeof DropObservationsSchema> = {
		...
		execute: (_id: string, params: DropObservationsArgs) => {
			toolCallCount++;
	...
	const context: AgentContext = { systemPrompt: DROPPER_SYSTEM, messages: [], tools: [dropObservations as AgentTool<TSchema>] };
