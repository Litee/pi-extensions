...
export const recallObservationTool = defineTool({
	...
	execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		if (!MEMORY_ID_PATTERN.test(memoryId)) {
			...
			return Promise.resolve(textResult(message, emptyDetails("invalid_id", memoryId, message)));
		}
		...
		if (result.status === "not_found") {
			...
			return Promise.resolve(textResult(message, emptyDetails("not_found", memoryId, message)));
		}
		return Promise.resolve(renderFoundResult(result));
	},
});
