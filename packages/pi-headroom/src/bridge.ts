...
export function buildCompressionPayload(messages: AgentMessage[], minMessageChars: number): CompressionPayload {
	...
	for (let sourceIndex = 0; sourceIndex < messages.length; sourceIndex++) {
		...
		const applyTo = originalText.length >= minMessageChars ? "compress" : null;
...
function replaceTextContent(message: AnyMessage, text: string, role: string): boolean {
	...
	if (role === "assistant") {
		// Replace text parts, preserve tool calls
		const toolCalls = message.content.filter((part): part is ToolCall => isToolCall(part));
		message.content = [{ type: "text", text }, ...toolCalls];
		return true;
	}

	// user, toolResult: replace text parts, preserve images
