...
import { applyCompressionResult, buildCompressionPayload } from "../src/bridge.ts";
import type { AgentMessage, OpenAIMessage } from "../src/types.ts";
...
describe("headroom bridge", () => {
	...
	it("applies compression to user messages when content changes", () => {
		const messages = [
		...
		expect(result.ok).toBe(true);
		if (!result.ok) return;
		const userMsg = result.messages[0] as Extract<AgentMessage, { role: "user" }>;
		expect(userMsg.content).toEqual([{ type: "text", text: "changed intent" }]);
	});
