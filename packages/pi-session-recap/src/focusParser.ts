...
/** Upper bound on the carry-over buffer; matches the safety net in `index.ts`. */
...
export function feedFocusBytes(
	...
	// Safety net — never let rest grow unbounded if the terminal streams a
	// long non-escape payload ahead of a potential focus sequence.
	if (rest.length > BUF_SAFETY_CAP) rest = rest.slice(-(MAX_SEQ - 1));
	// Suppress "assigned but never read" lint on `work` in strict builds.
	void work;
	return { events, rest };
}
