...
Two complementary triggers fire the recap; you get whichever fires first:

1. **Terminal focus reporting (DECSET `?1004`).** The extension enables
   focus events on session start and listens for `ESC[O` (focus-out) and
   `ESC[I` (focus-in). On focus-out it drafts a recap in the background;
   on focus-in it reveals the recap above the editor, as long as you were
   away for at least `--recap-focus-min-seconds` (default 3s).
...
The recap also fires automatically on `/resume` and `/fork`.

Clears on: next user input, new turn start, session reload, session
shutdown.
...
already have (including custom providers registered via
`pi.registerProvider`), so there are no login surprises.
