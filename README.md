...
| [`pi-cmux-update-workspace-name`](packages/pi-cmux-update-workspace-name) ⚠️ | Auto-renames the cmux workspace once per pi session based on an LLM summary of the first user prompt, gated on cmux's default `Terminal ` title prefix, plus a `/cmux-rename` command for on-demand regeneration. **Work in progress — not recommended for use.** |
| [`pi-context-window-analysis`](packages/pi-context-window-analysis) ⚠️ | Adds a `/context` command that shows a per-component token breakdown widget (system prompt, tools, context files, skills, conversation history, and last-turn actual API usage). **Work in progress — not recommended for use.** |
...
| [`pi-preset`](packages/pi-preset) | Named presets that configure model, thinking level, tools, system-prompt instructions, per-preset bash command filtering, and post-agent-end action prompts. Activated via `--preset`, `/preset`, or `Ctrl+Shift+U` to cycle. Ships with a `presets.example.json` reproducing plan + implement modes. |
