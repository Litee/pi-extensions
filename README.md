...
| [`pi-context-window-analysis`](packages/pi-context-window-analysis) | Adds a `/context` command that shows a per-component token breakdown widget (system prompt, tools, context files, skills, conversation history, and last-turn actual API usage). |
