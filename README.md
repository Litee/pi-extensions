...
| Package | Description |
|---------|-------------|
| [`pi-archon-workflow-watcher`](packages/pi-archon-workflow-watcher) | Watches archon workflow runs for status changes (paused=waiting for input, completed/failed/cancelled=done). Polls `archon workflow status` on a back-off schedule and injects one chat notification per status change. `archon_watcher` is inactive by default — activate with `manage_tools({"action":"activate","tools":["archon_watcher"]})` before use. Requires `pi-tools-management-tool`. |
