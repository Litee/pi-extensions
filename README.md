...
| [`pi-aws-s3-watcher`](packages/pi-aws-s3-watcher) | Watches a single AWS S3 object URI for existence, update, or removal via the AWS SDK. Fires exactly one chat notification when the target condition is met (or when an optional per-watch timeout elapses). Polls on a back-off schedule (60 s base, doubles to a 15 min cap; resets on any observable change). Auto-enabled — registers the `s3_watcher` tool (add/remove/list/pause/resume/status) from `session_start`. |
