...
| [`pi-aws-glue-watcher`](packages/pi-aws-glue-watcher) | Watches AWS Glue job and workflow runs for state changes and injects one chat notification per change. Registered at session_start but inactive — LLM activates via `manage_tools({action:"activate",tools:["glue_watcher"]})`, or user runs `/glue-watcher enable` as a manual escape hatch that also starts the widget + polling lifecycle. Polls `GetJobRun`/`GetWorkflowRun` on a 60 s → 15 min back-off; self-terminates on SUCCEEDED/FAILED/STOPPED/TIMEOUT/ERROR. |
