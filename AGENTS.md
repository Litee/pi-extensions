...
- **`agent_end` success:** the event is `{ type, messages }` — no status flag, and it fires even on abort/error. Detect a successful invocation via the last `role:"assistant"` message's `stopReason` ∉ `{"error","aborted"}` (set: `stop|length|toolUse|error|aborted`). Mocked unit tests won't reveal this.
- **Shared global state:** `~/.pi/agent/<name>.json` is shared by every concurrent pi session and they clobber each other. Key per-session state (counters, processed markers) by `ctx.sessionManager.getSessionId()`; reserve flat global fields for genuinely cross-session values.
...
- **Coverage improvement tasks:** When asked to raise branch coverage, the user wants me to **only add/extend tests** — never modify source files. Focus on the uncovered branches identified by the lcov report.
- **Coverage reporting:** Report before/after branch % for the targeted file(s) after adding tests.
- **Flat over deep:** Prefer flat directory structures over deep nesting. Organize code in a shallow layout when possible.
- **Descriptive filenames:** Use descriptive names (e.g. `list-all.ts`) instead of generic `index.ts`.
- **No comments unless explaining WHY:** Never comment code that explains HOW it works. Only add comments for WHY a non-obvious decision was made.
- **Short functions, no over-engineering:** Prefer short, focused functions. Avoid abstractions, interfaces, or patterns unless explicitly asked.

## Learned Workspace Facts
- The user runs systematic branch-coverage improvement tasks across multiple worktrees, following a naming pattern like `fix-cov1-*`, `fix-cov2-*`, `fix-cov3-*`, etc.
- The `pi-goal` package (`packages/pi-goal`) has been a target for coverage improvement efforts.
- The user prefers **bun** as the runtime/package manager (not npm), even though the repo's AGENTS.md instructs npm.
